        ...
        if i % print_freq == 0:
            ...
            try:
                if hasattr(model, "module"):
                    ...
                    mdl = model
                transformer = getattr(mdl, "transformer", None)
                get_stats = getattr(transformer.decoder, "get_router_stats", None) if transformer else None
                if callable(get_stats):
                    router_stats = get_stats()
                    # prefix to separate from loss metrics
                    for k, v in router_stats.items():
                        training_logs[f"stats/{k}"] = v
            except Exception:
                pass
...
def evaluate_acc(model, data_loader, epoch, accelerator=None):
    ...
    for images, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        if len(coco_evaluator.coco_eval["bbox"].cocoDt.dataset) == 0:
            continue  # No detections found in the dataset

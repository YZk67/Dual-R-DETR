# Apply compatibility patch for PyTorch versions that removed torch._six.string_classes
...
def train():
    ...
    for epoch in range(cfg.starting_epoch, cfg.num_epochs):
        # compute lambda(t) schedule per epoch
        if epoch < cfg.gating_warmup_epochs:
            gating_lambda = 0.0
        else:
            # progress in [0,1] over ramp
            ramp_pos = min(1.0, max(0.0, (epoch - cfg.gating_warmup_epochs) / max(1, cfg.gating_ramp_epochs)))
            # cosine ramp for smoother growth
            gating_lambda = float(cfg.gating_lambda_max * (0.5 - 0.5 * torch.cos(torch.tensor(ramp_pos * 3.1415926535)).item()))
        gating_last_n_layers = getattr(cfg, "gating_last_n_layers", 1)

        # set decoder gating knobs if available
        try:
            model.transformer.decoder.set_gating(lambda_value=gating_lambda, last_n_layers=gating_last_n_layers)
        except Exception:
            pass
        # set aux-alpha schedule for dual-branch training (configurable)
        try:
            # fixed value if provided
            if getattr(cfg, "aux_alpha_const", None) is not None:
                aux_alpha = float(cfg.aux_alpha_const)
            else:
                # cosine ramp from aux_alpha_min -> aux_alpha_max
                warm = int(getattr(cfg, "aux_alpha_warmup_epochs", cfg.gating_warmup_epochs))
                ramp = int(getattr(cfg, "aux_alpha_ramp_epochs", cfg.gating_ramp_epochs))
                a_min = float(getattr(cfg, "aux_alpha_min", 0.3))
                a_max = float(getattr(cfg, "aux_alpha_max", 0.7))
                if epoch < warm:
                    aux_alpha = a_min
                else:
                    ramp_pos = min(1.0, max(0.0, (epoch - warm) / max(1, ramp)))
                    aux_alpha = float(a_min + (a_max - a_min) * (0.5 - 0.5 * torch.cos(torch.tensor(ramp_pos * 3.1415926535)).item()))
            model.transformer.aux_alpha = float(aux_alpha)
        except Exception:
            pass
        # schedule router temperature: epochs [0,1] keep 1.5; ramp to 1.0 by epoch 5
        try:
            if epoch <= 1:
                router_tau = 1.5
            elif epoch <= 5:
                # linear decay from 1.5 to 1.0 across epochs 2-5
                router_tau = 1.5 - 0.5 * (epoch - 1) / (5 - 1)
            else:
                router_tau = 1.0
            model.transformer.decoder.set_router_tau(router_tau)
        except Exception:
            pass
        train_one_epoch_acc(
            model=model,
